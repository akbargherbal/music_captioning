...
def parse_job_file(job_path: str) -> list[dict]:
    ...
    for i, track in enumerate(tracks_raw):
        if "path" not in track:
            raise ValueError(
                f"Track at index {i} is missing required key 'path'."
            )
        ...
        max_tokens = int(merged.get("max_new_tokens", SCRIPT_DEFAULTS["max_new_tokens"]))
        ...
        resolved.append({
            ...
            "prompt_mode":    str(merged.get("prompt_mode",   SCRIPT_DEFAULTS["prompt_mode"])),
            "custom_prompt":  merged.get("custom_prompt")  or None,
            "output_dir":     str(merged.get("output_dir",   SCRIPT_DEFAULTS["output_dir"])),
            "tags":           merged.get("tags") if isinstance(merged.get("tags"), dict) else {},
        })
...
def main():
    ...
    # -----------------------------------------------------------------------
        ...
        for idx, audio_path in enumerate(audio_files, start=1):
            ...
            result = run_inference(model, processor, audio_path, prompt, args.max_tokens)
