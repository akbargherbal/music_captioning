...
def validate_audio_file(audio_path: str) -> tuple[bool, str or None]:
    ...
    preventing corrupt or oversized files from triggering permanent CUDA context failures.
    """
    try:
        import librosa

        # 1. Attempt to decode audio waveform natively (defaults to 16kHz mono)
        y, sr = librosa.load(audio_path, sr=16000)
        if y is None or len(y) == 0:
            return False, "Decoded audio waveform array is empty."

        # 2. Check for infinite or undefined floating values
        if np.isnan(y).any():
            return False, "Audio waveform array contains NaN values."
        if np.isinf(y).any():
            return False, "Audio waveform array contains infinite values."

        # 3. Check duration boundaries
        duration = librosa.get_duration(y=y, sr=sr)
        if duration <= 0:
            return False, f"Parsed audio duration is invalid ({duration:.2f}s)."
        
        # RoTE position embeddings in Music Flamingo support up to 20 minutes (1200 seconds)
        if duration > 1200:
            return False, f"Audio duration ({duration:.2f}s) exceeds maximum allowed length of 1200s (20 minutes)."

        return True, None
    except Exception as e:
        return False, f"Failed to parse or decode audio file: {str(e)}"
...
def run_inference(
    ...
    try:
        ...
        # Use autocast for bf16 models so mixed-precision ops resolve without manual dtype casting.
...
def main():
    ...
    if args.file:
        ...
        is_valid, err_msg = validate_audio_file(args.file)
        if not is_valid:
            sys.exit(f"Pre-flight Audio Validation failed: {err_msg}")
    ...
    # -----------------------------------------------------------------------
        ...
        for idx, audio_path in enumerate(audio_files, start=1):
            ...
            # Pre-flight Validation
            is_valid, err_msg = validate_audio_file(audio_path)
            if not is_valid:
